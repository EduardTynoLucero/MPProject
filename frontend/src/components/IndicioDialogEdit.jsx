...
export default function IndicioDialogEdit({ indicio, onClose, onSuccess }) {
  ...
  useEffect(() => {
    if (indicio) {
      setFormData({
        numero_evidencia: indicio.numero_evidencia || '',
        tipo: indicio.tipo || '',
        descripcion: indicio.descripcion || '',
        ubicacion: indicio.ubicacion || '',
        fecha_recoleccion: indicio.fecha_recoleccion?.split('T')[0] || '',
        cadena_custodia: indicio.cadena_custodia || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await indiciosAPI.update(indicio.id, formData)
      toast({ title: 'Éxito', description: 'Indicio actualizado correctamente' })
      onSuccess()
    } catch (error) {
      toast({
        title: 'Error',
        description: 'No se pudo actualizar el indicio',
        variant: 'destructive'
      })
    } finally {
      setLoading(false)
      ...
      <div className="relative mx-auto flex min-h-screen items-end justify-center p-3 sm:items-center sm:p-6">
        ...
        <div className="w-full overflow-hidden rounded-2xl border border-gray-100 bg-white/95 shadow-2xl shadow-indigo-100/40 backdrop-blur transition-all sm:max-w-2xl">
          ...
          <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4">
            <h3 className="text-base font-semibold text-gray-900">Editar Indicio</h3>
            <button
              onClick={onClose}
          ...
          <div className="max-h-[75vh] overflow-y-auto px-5 py-4 sm:px-6 sm:py-5">
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="flex flex-col-reverse gap-3 pt-2 sm:flex-row sm:justify-end">
                ...
                >
                  {loading ? 'Guardando...' : 'Actualizar Indicio'}
                </button>
              </div>
            </form>
          </div>
